...
const MealList = () => {
  ...
  const fetchUserData = async (userId) => {
    try {
      ...
      if (userDoc.exists()) {
        ...
        setUserNames((prev) => ({
          ...prev,
          [userId]: userData.name || "Anonymous User",
        }));
      }
    } catch (error) {
  ...
  useEffect(() => {
    const fetchMissingUserData = async () => {
      const mealsNeedingUserData = meals.filter(
        (meal) => meal.userId && !meal.user && !userNames[meal.userId]
      );

      // Fetch user data for each meal with a userId but no username
      for (const meal of mealsNeedingUserData) {
        await fetchUserData(meal.userId);
      }
    };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start sm:items-center mb-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="dropdown dropdown-end">
            ...
            <ul
              tabIndex={0}
              className="dropdown-content z-[1] menu p-2 shadow bg-neutral-800 rounded-box w-52"
            >
              <li>
                <a onClick={() => handleCategoryChange("")}>All Categories</a>
              </li>
              <li>
                <a onClick={() => handleCategoryChange("Italian")}>Italian</a>
              </li>
              <li>
                <a onClick={() => handleCategoryChange("Chinese")}>Chinese</a>
              </li>
              <li>
                <a onClick={() => handleCategoryChange("Mexican")}>Mexican</a>
              </li>
              <li>
                <a onClick={() => handleCategoryChange("Indian")}>Indian</a>
              </li>
              <li>
                <a onClick={() => handleCategoryChange("Thai")}>Thai</a>
              </li>
              <li>
                <a onClick={() => handleCategoryChange("Mediterranean")}>
                  Mediterranean
                </a>
              </li>
              <li>
                <a onClick={() => handleSortChange("createdAt", "desc")}>
                  Newest First
                </a>
              </li>
              <li>
                <a onClick={() => handleSortChange("macros.calories", "asc")}>
                  Calories (Low to High)
                </a>
              </li>
              <li>
                <a onClick={() => handleSortChange("macros.protein", "desc")}>
                  Protein (High to Low)
                </a>
              </li>
            </ul>
      ...
      {(categoryFilter || sortOption.field !== "createdAt") && (
        <div className="mb-4 flex flex-wrap gap-2">
          ...
          {sortOption.field !== "createdAt" && (
            <span className="bg-primary/20 text-primary px-3 py-1 text-sm rounded-full flex items-center">
              Sort:{" "}
              {sortOption.field.includes("macros")
                ? sortOption.field.split(".")[1]
                : sortOption.field}
      ...
      {isLoading && meals.length === 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[...Array(itemsPerPage)].map((_, i) => (
            <div
              key={i}
              className="bg-neutral-800/50 rounded-xl shadow-lg animate-pulse h-80"
            ></div>
          ))}
        </div>
      ) : meals.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {meals.map((meal, index) => (
            <div
              key={meal.id}
              className={`bg-neutral-800 rounded-xl shadow-lg overflow-hidden transform transition-all duration-500 ${
              ...
              <Link href={`/meal/${meal.id}`}>
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${getMacroColor(
                        "calories",
