...
/**
 * Add a meal to the user's taken_macros for today
 * @param {string} userId - The user's ID
 * @param {object} meal - The meal object with name, macros, etc.
 */
export const addMealToTracking = async (userId, meal) => {
  try {
    // Format today's date
    const today = new Date().toISOString().split('T')[0];
    
    // Reference to today's taken_macros document
    const takenMacrosRef = doc(db, "users", userId, "taken_macros", today);
    ...
    if (takenMacrosDoc.exists()) {
      ...
      const currentTotal = data.total || { calories: 0, protein: 0, carbs: 0, fat: 0 };
      
      // Calculate new totals
      const newTotal = {
        calories: (currentTotal.calories || 0) + (meal.macros.calories || 0),
        protein: (currentTotal.protein || 0) + (meal.macros.protein || 0),
        carbs: (currentTotal.carbs || 0) + (meal.macros.carbs || 0),
        fat: (currentTotal.fat || 0) + (meal.macros.fat || 0)
      };
      
      // Update the document with new meal and updated totals
    ...
    console.error("Error adding meal to tracking:", error);
    throw error;
  }
};